...
- Each language are in its own folder, e.g. `sv/` is for Swedish

- In there a folder for each topic contains markdown files,
  e.g. `sv/Omraade-Energifoerluster/Ledning_laerare.md` contains one exercise
  
In order to add a new exercise:

- Add a new markdown file in the topic folder,
  e.g. `sv/Omraade-Energifoerluster/newexercise_leaere.md`
  
- To add it to the swedish page the open the `.Rmd` file in the main directory, e.g. `teachers_GN_sv.Rmd` in Rstudio:

    - Add a line with the link to the markdown file by copying a line with a
	    link to the pages and change the link, as in:
	    \'r knitr::knit_child('sv/Omraade-Energifoerluster/newexercise.md')\`
...
- Keep the heading levels in the exercise files (see e.g. `sv/Omraade-Energifoerluster/Ledning_laerare.md`):
    - `### Exercise title`
	- `#### Subheadings`
...
- Open an exercise markdown file,
  e.g. `sv/Omraade-Energifoerluster/Ledning_laerare.md`
